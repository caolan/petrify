...
exports.withFiles = function(dirname, pattern, fn, callback){
    fs.readdir(dirname, function(err, files){
        ...
        files.forEach(function(filename){
            fs.readFile(path.resolve(dirname, filename), function(err, data){
                if(err){
                    callback(err);
    ...
    fs.readdir(dirname, function(err, files){
        ...
        emitter.emit('loaded', files.reduce(function(a,x){
            ...
            if(/\.js$/.exec(basename)){
                ...
                try {
                    a[module_name] = require(
                        path.resolve(dirname, module_name)
                    );
                    completed++;
...
exports.loadTemplates = function(template_dir){
    var emitter = new events.EventEmitter();
    var templates = {};
    exports.withFiles(template_dir, /^[^\.]+.*$/, function(filename, data){
        var cache;
        templates.__defineGetter__(filename, function(){
            if(!cache){
                cache = handlebars.compile(data);
            }
            return cache;
        });
    }, function(err){
...
exports.run = function(opt){
    ...
    async.auto({
        ...
        removedir: function(callback){
            fs.exists(opt.output_dir, function(exists){
                if(exists){
