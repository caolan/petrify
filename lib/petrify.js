        ...
        files.forEach(function(filename){
            fs.readFile(path.join(dirname, filename), function(err, data){
                if(err){
                    ...
                    fn(filename, data, completed, total);
                }
                waiting -= 1;
                if(!waiting){
                    callback(null);
...
exports.loadViews = function(dirname){
    ...
    var completed = 0;
    fs.readdir(dirname, function(err, files){
        if(err) return emitter.emit('finished', err, null);
        if(!files.length) return emitter.emit('finished', null, {});
        emitter.emit('finished', null, files.reduce(function(a,x){
            ...
            if(/\.js$/.exec(basename)){
                ...
                completed++;
                emitter.emit('loaded', module_name, completed, files.length);
            }
            return a;
        }, {}));
    });
...
exports.loadData = function(dirname){
    ...
    exports.withFiles(
        ...
        function(filename, fileData, completed, total){
            ...
            emitter.emit('loaded', filename, completed, total);
        },
        function(err){
            emitter.emit('finished', err, data);
        }
    );
    return emitter;
};
...
exports.loadTemplates = function(template_dir){
    ...
    exports.withFiles(template_dir, /.*\.jsont$/, function(filename, data){
        ...
        process.nextTick(function(){
            emitter.emit('finished', err, templates);
        });
    });
    return emitter;
};
...
exports.run = function(opt){
    ...
    run.auto({
        templates: function(task){
            var loadTemplates = exports.loadTemplates(opt.template_dir);
            loadTemplates.addListener('finished', function(err, templates){
                if(err) sys.puts(err);
                else {
        ...
        views: function(task){
            var loadViews = exports.loadViews(opt.view_dir);
        ...
        data: function(task){
            var loadData = exports.loadData(opt.data_dir);
