...
exports.loadViews = function(dirname){
    ...
    var completed = 0;
    fs.readdir(dirname, function(err, files){
        if(err) return emitter.emit('loaded', err, null);
        if(!files.length) return emitter.emit('loaded', null, {});
        emitter.emit('loaded', null, files.reduce(function(a,x){
...
exports.loadTemplates = function(template_dir){
    ...
    exports.withFiles(template_dir, /.*\.jsont$/, function(filename, data){
        ...
        process.nextTick(function(){
            emitter.emit('loaded', err, templates);
        });
    });
    return emitter;
};
...
exports.run = function(opt){
    ...
    run.auto({
        templates: function(task){
            ...
            emitter.templates.addListener('loaded', function(err, templates){
                if(err) sys.puts(err);
                else {
        ...
        views: function(task){
            ...
            emitter.views.addListener('loaded', function(err, views){
                if(err) sys.puts(err);
                else {
                    opt.views = views;
                    task.done();
        ...
        data: function(task){
            ...
            emitter.data.addListener('loaded', function(err, data){
                if(err) sys.puts(err);
                else {
                    opt.data = data;
                    task.done();
        ...
        runViews.addListener('finished', function(err){
            emitter.views.emit('finished', err);
