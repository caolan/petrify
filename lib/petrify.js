    ...
    var tasks = Object.keys(opts.views).reduce(function(a,k){
        ...
        a[k] = run.requires(view.requires, function(task){
            ...
            try {
                ...
                view.run(viewEnv, opts.templates, opts.data, partials);
            }
            catch(e){
                callback(e, null);
            }
...
exports.emit = function(output_dir, urlpath, data, callback){
    var filename = output_dir + '/' + urlpath.replace(/^\//, '');
    ...
    fs.readdir(template_dir, function(err, files){
        ...
        files.forEach(function(file){
            fs.readFile(path.join(template_dir, file), function(err, data){
                ...
                var cache;
                templates.__defineGetter__(file, function(){
                    cache = cache || new jsontemplate.Template(data);
                    return cache;
                });
                waiting -= 1;
                if(!waiting){
